...
#
# Usage: ./submit-snolab.sh <source> <Nevents> <threads=5>
#
...
dir=$(dirname $0)	# Directory to find run-snolab.sh script
...
sbatch -J $src -t "${hours}:00:00" --cpus-per-task=$ncores $dir/run-snolab.sh $src $nevt $nthread
