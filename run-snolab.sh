#!/bin/sh
#
# Usage: run-snolab.sh <source> <events> [threads=5]
# Generate <source> events in SuperCDMS SNOLAB, including G4DMC
# Uses SimProdMacros directly; get number of events from command line
# Number of worker threads may be specified optionally; default is 5
#
...
set -e
time runSourceSim $src -f snolab $opts -n $evt -r $run -t $thr -o `pwd -P` -E
