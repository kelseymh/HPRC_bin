...
#
# Usage: ./submit.sh <source> <Nevents> <threads=15>
#
...
dir=$(dirname $0)	# Directory to find run-bulk.sh script

src=$1
nevt=${2:-100}		# Number of events, use to compute job time, etc.
nthread=${3:-15}	# Number of worker threads
...
sbatch -J $src -t "${hours}:00:00" --cpus-per-task=$ncores $dir/run-bulk.sh $src $nevt $nthread
